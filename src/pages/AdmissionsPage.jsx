...
const WHATSAPP_NUMBER = '97444215000';
...
const requirements = {
  EN: ['Birth certificate (original + copy)', 'Previous school transcripts (last 2 years)', 'Passport copy (student & parents)', 'Recent passport-size photographs (4 copies)', 'Medical health records', 'Proof of residency in Qatar'],
  AR: ['شهادة الميلاد (أصل + نسخة)', 'كشف درجات المدرسة السابقة (آخر سنتين)', 'نسخة من جواز السفر (الطالب والوالدين)', 'صور شخصية حديثة بحجم جواز السفر (4 نسخ)', 'السجلات الطبية', 'إثبات الإقامة في قطر'],
};

const questions = {
  EN: [
    { id: 'childName', label: "What is your child's full name and date of birth?", placeholder: "e.g. Mohammed Al-Ahmad, 15 March 2012" },
    { id: 'grade', label: "Which grade are you applying for?", placeholder: "e.g. Grade 9 (Year 10)" },
...
export default function AdmissionsPage() {
  ...
  const directWhatsApp = () => {
    const msg = isAR
      ? encodeURIComponent('مرحباً، أود الاستفسار عن القبول في مدرسة الدوحة الثانوية للعام الدراسي 2025-2026.')
      : encodeURIComponent('Hello, I would like to inquire about admissions to Doha High School for the 2025–2026 academic year.');
    window.open(`https://wa.me/${WHATSAPP_NUMBER}?text=${msg}`, '_blank');
  };
      ...
      <section className="tuition-section">
        <div className="container">
          <div className="section-header reveal">
            <span className="section-label">{isAR ? 'الرسوم الدراسية' : 'Tuition & Fees'}</span>
            <h2 className="section-title">{isAR ? 'شفافية في التكاليف' : 'Transparent Pricing'}</h2>
            <div className="section-divider center" />
          </div>
          <div className="tuition-grid">
            {[
              { level: isAR ? 'المرحلة الابتدائية' : 'Primary (Grades 1–5)', fee: 'QAR 18,000', period: isAR ? 'سنوياً' : 'per year' },
              { level: isAR ? 'المرحلة الإعدادية' : 'Middle (Grades 6–8)', fee: 'QAR 22,000', period: isAR ? 'سنوياً' : 'per year' },
              { level: isAR ? 'مسار IGCSE' : 'IGCSE Track (Grades 9–10)', fee: 'QAR 27,000', period: isAR ? 'سنوياً' : 'per year', featured: true },
              { level: isAR ? 'مسار IB' : 'IB Diploma (Grades 11–12)', fee: 'QAR 32,000', period: isAR ? 'سنوياً' : 'per year', featured: true },
            ].map((t, i) => (
              <div className={`tuition-card reveal delay-${i + 1} ${t.featured ? 'featured' : ''}`} key={i}>
                {t.featured && <div className="featured-badge">{isAR ? 'الأكثر شعبية' : 'Most Popular'}</div>}
                <div className="tuition-level">{t.level}</div>
                <div className="tuition-fee">{t.fee}</div>
                <div className="tuition-period">{t.period}</div>
                <Link to="/contact" className={t.featured ? 'btn-primary' : 'btn-outline'} style={!t.featured ? { color: 'var(--navy)', borderColor: 'var(--navy)' } : {}}>
                  {isAR ? 'استفسر الآن' : 'Inquire Now'}
                </Link>
              </div>
            ))}
          </div>
          <p className="tuition-note reveal">
            {isAR
              ? '* قد تختلف الرسوم. يرجى التواصل مع فريق القبول للحصول على تفاصيل دقيقة وفرص المنح الدراسية.'
              : '* Fees may vary. Please contact our admissions team for exact details and scholarship opportunities.'}
          </p>
        </div>
      </section>
      ...
      <section className="faq-section">
        <div className="container">
          <div className="section-header reveal">
            ...
            <h2 className="section-title">{isAR ? 'الأسئلة الشائعة' : 'Frequently Asked Questions'}</h2>
            <div className="section-divider center" />
          </div>
          <div className="faq-grid">
            {(isAR ? [
              { q: 'ما هي متطلبات الالتحاق؟', a: 'يتطلب الالتحاق تقديم كشف الدرجات السابق وشهادة الميلاد وجواز السفر وصور شخصية. وقد يُطلب منك الخضوع لتقييم مستوى.' },
              { q: 'هل تتوفر منح دراسية؟', a: 'نعم، تقدم المدرسة منحاً دراسية جزئية للطلاب المتفوقين. تواصل مع فريق القبول لمعرفة التفاصيل.' },
              { q: 'متى يبدأ العام الدراسي؟', a: 'يبدأ العام الدراسي عادةً في سبتمبر ويتضمن فصلين دراسيين.' },
              { q: 'هل المدرسة مختلطة؟', a: 'نعم، مدرسة الدوحة الثانوية مدرسة مختلطة لجميع الفئات العمرية.' },
            ] : [
              { q: 'What are the enrollment requirements?', a: 'Enrollment requires previous school transcripts, birth certificate, passport copy, and passport photos. A placement assessment may be required.' },
              { q: 'Are scholarships available?', a: 'Yes, the school offers partial scholarships for high-achieving students. Contact the admissions team for details.' },
              { q: 'When does the academic year start?', a: 'The academic year typically starts in September and runs in two semesters.' },
              { q: 'Is the school co-educational?', a: 'Yes, Doha High School is co-educational for all age groups.' },
            ]).map((item, i) => (
              <FaqItem key={i} q={item.q} a={item.a} delay={i + 1} />
            ))}
          </div>
        </div>
      </section>
...
function FaqItem({ q, a, delay }) {
  ...
  return (
    <div className={`faq-item reveal delay-${delay} ${open ? 'open' : ''}`}>
      <button className="faq-question" onClick={() => setOpen(!open)}>
        <span>{q}</span>
        <svg className="faq-arrow" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M6 9l6 6 6-6"/></svg>
      </button>
